"""
Rebuild the Master Excel and CSV files with perfect alignment, clean formatting, and green row highlights
"""
...
KNOWN_TOGGLE_ON = {
    "@chaitra_vasudevan_official_", "@rashwin99", "@prithinarayanan", "@aarya.lakshmi_"
}
...
for s in brand_sheets:
    ...
    for r in range(3, ws.max_row + 1):
        ...
        if "owned by partner" in via.lower():
            ...
            # Check Toggle ON status
            cap_l = cap.lower()
            is_on = (h.lower() in KNOWN_TOGGLE_ON) or any(t in cap_l for t in ["#ad", "#sponsored", "#paidpartnership", "paid partnership"])
...
print(f"Total Paid Collabs Extracted: {len(all_paid_posts)}")
...
ws_s1["A1"] = f"Executive Summary — Paid Creator Partnerships by Jewellery Brand ({len(all_paid_posts)} Total Posts · {unique_creators_total} Unique Creators)"
...
on_tot = sum(1 for p in all_paid_posts if p["is_on"])
off_tot = len(all_paid_posts) - on_tot

ws_s2.merge_cells("A1:L1")
ws_s2["A1"] = f"Consolidated Creator-Owned Paid Partnerships ('Post owned by partner') — {len(all_paid_posts)} Total Posts (Toggle ON: {on_tot} · Toggle OFF: {off_tot})"
...
collab_cols = [
    ("#", 5), ("Brand Name", 28), ("Creator Handle", 25), ("Paid Partnership Toggle", 22),
...
print(f"\n✅ Successfully generated master files:")
print(f"   1. Excel: {EXCEL_OUTPUT} (Total sheets: {len(wb_out.sheetnames)})")
print(f"   2. CSV:   {CSV_COLLABS} (1,067 rows with Toggle column)")
print(f"   3. CSV:   {CSV_SUMMARY} (34 brands summary)")
